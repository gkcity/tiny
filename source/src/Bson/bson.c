...
void bson_copy(bson *out, const bson *in)
{
    ...
    out->data = tiny_malloc((size_t) bson_size(in));
    out->owned = 1;
    memcpy(out->data, in->data, (size_t) bson_size(in));
}
...
{
    b->buf = (char *) tiny_malloc(initialBufferSize);
...
bson_buffer *bson_ensure_space(bson_buffer *b, const int bytesNeeded)
{
    ...
    if (b->finished)
    {
        LOG_E(TAG, "trying to append to finished buffer");
    }
...
{
    if (!bson_append_estart(b, bson_int, name, 4))
    {
        return 0;
    }
    bson_append32(b, &i);
...
{
    if (!bson_append_estart(b, bson_long, name, 8))
    {
        return 0;
    }
    bson_append64(b, &i);
...
{
    if (!bson_append_estart(b, bson_double, name, 8))
    {
        return 0;
    }
    bson_append64(b, &d);
...
{
    if (!bson_append_estart(b, bson_bool, name, 1))
    {
        return 0;
    }
...
{
    if (!bson_append_estart(b, bson_null, name, 0))
    {
        return 0;
    }
    return b;
...
{
    if (!bson_append_estart(b, bson_undefined, name, 0))
    {
        return 0;
    }
    return b;
...
{
    if (!bson_append_estart(b, bson_oid, name, 12))
    {
        return 0;
    }
...
{
    if (!bson_append_estart(b, bson_date, name, 8))
    {
        return 0;
    }
...
{
    if (!bson_append_estart(b, bson_object, name, 5))
    {
        return 0;
    }
...
{
    if (!bson_append_estart(b, bson_array, name, 5))
    {
        return 0;
    }
    b->stack[b->stackPos++] = b->cur - b->buf;
