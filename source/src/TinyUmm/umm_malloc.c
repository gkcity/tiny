...
// ----------------------------------------------------------------------------
//
// The memory manager assumes the following accessories:
//
...
static umm_block umm_heap[(UMM_MALLOC_CFG__HEAP_SIZE / sizeof(umm_block))];
static const unsigned short int umm_numblocks = (sizeof(umm_heap) / sizeof(umm_block));
...
// ----------------------------------------------------------------------------
// One of the coolest accessories about this little library is that it's VERY
...
void *umm_info(void *ptr, int force)
{
    ...
    LOG_D(TAG, "Dumping the umm_heap...");
    LOG_D(TAG, "|0x%08x|B %5i|NB %5i|PB %5i|Z %5i|NF %5i|PF %5i|",
    ...
    blockNo = (unsigned short int)(UMM_NBLOCK(blockNo) & UMM_BLOCKNO_MASK);
    ...
    while (UMM_NBLOCK(blockNo) & UMM_BLOCKNO_MASK)
    {
        ...
        else
        {
            ...
            LOG_D(TAG, "|0x%08x|B %5i|NB %5i|PB %5i|Z %5i|",
                          (unsigned int) (&UMM_BLOCK(blockNo)),
                          blockNo,
                          UMM_NBLOCK(blockNo) & UMM_BLOCKNO_MASK,
                          UMM_PBLOCK(blockNo),
    ...
    LOG_D(TAG, "Total Entries %5i    Used Entries %5i    Free Entries %5i",
                  heapInfo.totalEntries,
                  heapInfo.usedEntries,
                  heapInfo.freeEntries);

    LOG_D(TAG, "Total Blocks  %5i    Used Blocks  %5i    Free Blocks  %5i",
...
static unsigned short int umm_blocks(size_t size)
{
    ...
    // a few little accessories that we need to be mindful of.
    //
...
{
    UMM_NBLOCK(c + blocks) = (unsigned short int)(UMM_NBLOCK(c) & UMM_BLOCKNO_MASK);
...
static void umm_assimilate_up(unsigned short int c)
{
    if (UMM_NBLOCK(UMM_NBLOCK(c)) & UMM_FREELIST_MASK)
    {
        ...
        LOG_D(TAG, "Assimilate up to next block, which is FREE");
        ...
        UMM_NBLOCK(c) = (unsigned short int)(UMM_NBLOCK(UMM_NBLOCK(c)) & UMM_BLOCKNO_MASK);
    }
}
...
void umm_free(void *ptr)
{
    ...
    if ((void *) 0 == ptr)
    {
        LOG_D(TAG, "free a null pointer -> do nothing");
        return;
    }
    ...
    //c = (unsigned short int)((ptr - (void *) (&(umm_heap[0]))) / sizeof(umm_block));
	c = (unsigned short int)(((char *)ptr - (char *)(&(umm_heap[0]))) / sizeof(umm_block));

    LOG_D(TAG, "Freeing block %6i", c);
    ...
    if (UMM_NBLOCK(UMM_PBLOCK(c)) & UMM_FREELIST_MASK)
    {
        ...
        LOG_D(TAG, "Assimilate down to next block, which is FREE");
    ...
    else
    {
        ...
        LOG_D(TAG, "Just add to head of free list");
...
{
    unsigned short int blocks = 0;
    /* volatile --COMMENTED BY DFRANK because the version from FreeRTOS doesn't have it*/
    unsigned short int blockSize = 0;
    unsigned short int bestSize = 0;
    unsigned short int bestBlock = 0;
    ...
    if (0 == size)
    {
        LOG_D(TAG, "malloc a block of 0 bytes -> do nothing");
        return ((void *) NULL);
    }
    ...
    while (UMM_NFREE(cf))
    {
        blockSize = (unsigned short int)((UMM_NBLOCK(cf) & UMM_BLOCKNO_MASK) - cf);

        LOG_D(TAG, "Looking at block %6i size %6i", cf, blockSize);
    ...
    if (UMM_NBLOCK(cf) & UMM_BLOCKNO_MASK)
    {
        ...
        if (blockSize == blocks)
        {
            ...
            LOG_D(TAG, "Allocating %6i blocks starting at %6i - exact", blocks, cf);
            // Disconnect this block from the FREE list
        ...
        else
        {
            ...
            LOG_D(TAG, "Allocating %6i blocks starting at %6i - existing", blocks, cf);
    ...
    else
    {
        ...
        {
            LOG_D(TAG, "Can't allocate %5i blocks at %5i", blocks, cf);
            // Release the critical section...
            UMM_CRITICAL_EXIT();
            return ((void *) NULL);
        }
        ...
        if (0 == cf)
        {
            LOG_D(TAG, "Initializing malloc free block pointer");
            UMM_NBLOCK(0) = 1;
            UMM_NFREE(0) = 1;
            cf = 1;
        }

        LOG_D(TAG, "Allocating %6i blocks starting at %6i - new     ", blocks, cf);
...
void *umm_realloc(void *ptr, size_t size)
{
    ...
    if (((void *) NULL == ptr))
    {
        LOG_D(TAG, "realloc the NULL pointer - call malloc()");
    ...
    if (0 == size)
    {
        LOG_D(TAG, "realloc to 0 size, just free the block");
        umm_free(ptr);
        return ((void *) NULL);
    }
    ...
    if (blockSize == blocks)
    {
        ...
        LOG_D(TAG, "realloc the same size block - %i, do nothing", blocks);
        // Release the critical section...
        UMM_CRITICAL_EXIT();
        return (ptr);
    }
    ...
    if ((UMM_NBLOCK(UMM_PBLOCK(c)) & UMM_FREELIST_MASK) && (blocks <= (UMM_NBLOCK(c) - UMM_PBLOCK(c))))
    {
        // Check if the resulting block would be big enough...
        LOG_D(TAG, "realloc() could assimilate down %i blocks - fits!", c - UMM_PBLOCK(c));
    ...
    else if (blockSize > blocks)
    {
        ...
        LOG_D(TAG, "realloc %i to a smaller block %i, shrink and free the leftover bits", blockSize, blocks);
    ...
    else
    {
        ...
        void *oldptr = ptr;
        LOG_D(TAG, "realloc %i to a bigger block %i, make new, copy, and free the old", blockSize, blocks);
